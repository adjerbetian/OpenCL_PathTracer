...
namespace PathTracerNS
{
	...
	void PathTracerFileImporter::Export()
	{
		...
		if(fichier != NULL)
		{
			...
			nWritte = fwrite(ptr__global__cameraScreenX		, sizeof(Float4), 1, fichier);
			nWritte = fwrite(ptr__global__cameraScreenY		, sizeof(Float4), 1, fichier);
	...
	void PathTracerFileImporter::Import()
	{
		...
		if(fichierSizes != NULL)
		{
			...
			nRead = fread(ptr__global__cameraScreenX	, sizeof(Float4), 1, fichierSizes);
			nRead = fread(ptr__global__cameraScreenY	, sizeof(Float4), 1, fichierSizes);
