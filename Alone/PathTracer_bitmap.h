...
namespace PathTracerNS
{
	...
	BYTE* ConvertRGBAToBMPBuffer ( RGBAColor const * const * imageColor, uint const * const * imageRay, int width, int height, long* newsize )
	{
		...
		*newsize = height * psw;
		...
		for ( int y = 0; y < height; y++ )
		{
			...
			{
				int nRays = max(imageRay[x][y], 1);

				int r,g,b;

				if( (imageColor[x][y].x < 0) || (imageColor[x][y].x < 0) || (imageColor[x][y].x < 0) )
				{
					r = 255;
					g = 0;
					b = 0;
				...
				{
					r = (int) min(imageColor[x][y].x * 255.f / nRays, 255.f);
					g = (int) min(imageColor[x][y].y * 255.f / nRays, 255.f);
					b = (int) min(imageColor[x][y].z * 255.f / nRays, 255.f);
				}
